...
Article
[How to Manage Connections Efficiently in Postgres, or Any Databaseð](/postgres-connections)
Published
OctÂ 15,Â 2018
Location
San Francisco
See comments and reaction
**on Hacker News**
.
...
OctÂ 15,Â 2018
1. The practical limits of concurrency
2. Techniques for efficient connection useConnection poolsMinimum viable checkoutsReleasing connections around foreign mutationsPgBouncer & inter-node pooling
3. Connections as a resource
...
The ceiling is controlled by themax_connectionskey in
...
Likewise,GetSnapshotDatais often called multiple times
...
backends in the wider system. I wrote abenchmarkto
...
deletes ten times before committing2. Parallelism starts
...
built into many database adapters including Go’sdatabase/sql, Java’sJDBC, or
...
pool while that core logic is executing. Thisminimum
viable checkouttechnique maximizes the efficient use of
...
work intoatomic phasesaround where an
...
If we haveNnodes andMmaximum connections per node,
we may have a configuration whereNÃMis greater than
the database’smax_connectionsto protect against the
...
In this case it’s possible to installPgBouncerto act as a global pool by proxying
...
state of a connection likeSET,LISTEN/NOTIFY, or
...
configured with anNÃMgreater thanmax_connectionsto closely approach the maximum possible theoretical
...
1Andres Freundnotes that it’s possible
to substantially reduce per-backend memory overhead by
making sure thathuge pagesare enabled.
Long-lived children eventually end up copying (in the sense
ofCOW) most of their parent’s page table into their
...
2Each transaction leaves its target table empty to avoid
...
3Threaded deployments in Ruby are possible, but because
of Ruby’s GIL (global interpreter lock), they’ll be
fundamentally slower than using a forking process model.

4It may be possible to use prepared statements with
...
Did I make a mistake? Please considersending a
                    pull request.