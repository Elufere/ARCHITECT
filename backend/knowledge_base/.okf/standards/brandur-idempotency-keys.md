...
Article
[Implementing Stripe-like Idempotency Keys in Postgresð](/idempotency-keys)
Published
OctÂ 27,Â 2017
Location
San Francisco
...
OctÂ 27,Â 2017
1. Idempotency with keys
2. Rocket RidesThe request lifecycleThe entropy of production
3. Foreign state mutationsBetween any two systems
4. Atomic phasesRecovery pointsBackground jobs and staging
5. Hardening Rocket Rides for interstellar travelThe idempotency key relationOther schemaDesigning atomic phasesAtomic phase implementationIdempotency key upsertA directed and acyclic state machineInitial bookkeepingCalling StripeSend receipt and finish
6. Other processesThe enqueuerThe completerThe reaper
7. Murphy in action
8. ComplicationsNon-idempotent foreign state mutationsNon-ACID data stores
9. Beyond APIs
10. Cultivating passive safety

In APIsidempotencyis a powerful concept. An
...
mapping requests to transactions which I wrote aboutin
more detail a few weeks ago. This
...
through, the client retries withthe sameidempotency key
...
calledRocket Ridesto demonstrate the use
of the Connect platform and other interesting parts of the
API. In Rocket Rides, users who are in a hurry share a ride
with a jetpack-certified pilot to get where they’re goingfast. SOMA’s gridlock traffic disappears into the
...
TheRocket Rides repositorycomes with a
...
makingforeign state mutations; that is, calling out
...
committed one way or another1.We’ve pushed data into a
...
Anatomic phaseis a set of local state mutations
that occur in transactionsbetweenforeign state
...
Atomic phases should be safely committedbeforeinitiating any foreign state mutation. If the call fails,
our local state will still have a record of it happening
that we can use to retry the operation.

### Recovery points

Arecovery pointis a name of a check point that we
get to after having successfully executed any atomic phaseorforeign state mutation. Its purpose is to allow a
...
a recovery point ofstarted, and after any request is
complete (again, through either a success or definitive
error) it’ll be assigned a recovery point offinished.
...
In our Rocket Rides example the charge to Stripe probablycan’tbe deferred – we want to know whether it succeeded
right away so that we can deny the request if it didn’t.
Sending an emailcanand should be sent to the
background.

By using atransactionally-staged job
drain, we can hide jobs from workers until
...
needed to make everything work (seeAtomic Rocket Ride’sProcfilefor the full list and the
corresponding implementations in the same repository).

### The enqueuer

There should be anenqueuerthat moves jobs fromstaged_jobsto the job queue after their inserting
transaction has committed. Seethis articlefor
details on how to build one, orthe
implementationfromAtomic Rocket Rides.
...
A stretch goal is to implement acompleter. Its only
...
See theAtomic Rocket Ridesrepository fora completer
implementation.
...
historical requests. After some amount of time areaperprocess should go through keys and delete
them.
...
See theAtomic Rocket Ridesrepository fora reaper
implementation.
...
### Non-idempotent foreign state mutations
...
operations commit atomically –everyoperation against
...
## Cultivating passive safety

API backends should aim to bepassively safe– no matter
...
Purely idempotent transactionsand
...
1There is one caveat that it may be possible to
implementtwo-phase commitbetween a system and all
