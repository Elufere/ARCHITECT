...
def build_graph() -> StateGraph:
    ...
    workflow.add_node("activate_requirements", durable_node("activate_requirements", requirement_activation_node, lambda _: "plan"))
    ...
    workflow.add_conditional_edges(START, resume_at, {
        name: name for name in ("conversation_manager", "extract", "activate_requirements", "plan", "generate", "guardrail", "compile_prd", END)})
    ...
    # 3. Extract -> Requirement activation -> Plan
    workflow.add_edge("extract", "activate_requirements")
    workflow.add_edge("activate_requirements", "plan")
